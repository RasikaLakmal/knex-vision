...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("knex-vision.preview", async (arg: any) => {
      let targetUri: vscode.Uri | undefined;

      // Handle TreeItem
      if (arg && arg.migration && arg.migration.path) {
        targetUri = vscode.Uri.file(arg.migration.path);
      ...
      if (targetUri) {
        ...
        try {
          ...
          if (migration.diff && migration.diff.tables) {
            migration.diff.tables.forEach((t) => tableNames.push(t.tableName));
          }
